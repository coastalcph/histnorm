...
def main(args):
    for line in args.infile:
        ...
        if args.filter_punctuation:
            punct = [all(map(lambda x: unicodedata.category(x).startswith('P'), t)) \
                             for t in tokens if len(t) > 0]
            if any(punct):
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', '--filter-boundaries',
                        action='store_true',
                        default=False,
    ...
    parser.add_argument('-u', '--unicode-normalize',
